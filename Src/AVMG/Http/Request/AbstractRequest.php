...
use
    ...
    AVMG\Http\Helper\RequestMethod;
/** ***********************************************************************************************
...
abstract class AbstractRequest extends AbstractMessage implements RequestInterface
{
    ...
    public function __construct(string $method, UriInterface $uri)
    {
        ...
        try
        {
            $this->method = RequestMethod::normalize($method);
        }
    ...
    public function withMethod(string $method) : RequestInterface
    {
        ...
        {
            $newInstance = clone $this;
            $newInstance->method = RequestMethod::normalize($method);
    ...
    public function withUri(UriInterface $uri, bool $preserveHost = false) : RequestInterface
    {
        ...
        if ($preserveHost)
        {
            try
            {
                $newInstance = $newInstance->withHeader
                (
                    self::HOST_HEADER_NAME,
                    $this->getHostFromUri($uri)
                );
            }
            catch (InvalidArgumentException $exception)
            {
    ...
    protected function getRequestTargetFromUri(UriInterface $uri) : string
    {
        ...
        return strlen($query) > 0 && strlen($path) > 0
            ? "$path?$query"
            : $path;
    }
    /** **********************************************************************
    ...
    protected function getHostFromUri(UriInterface $uri) : string
    {
        ...
        return strlen($host) > 0 && !is_null($port)
            ? "$host:$port"
            : $host;
    }
    /** **********************************************************************
